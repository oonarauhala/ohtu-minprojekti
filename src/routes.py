...
def new_podcast():
   ...
   if request.method == "POST":
      ...
      kommentti = request.form["kommentti"]
      if len(nimi.split())==0 or len(tekija.split())==0 or len(jakson_nimi.split())==0 or len(kommentti.split())==0:
         return render_template("error.html", viesti="Kaikki kentät tulee täyttää")
...
def new_video():
   ...
   if request.method == "POST":
      ...
      url = request.form["url"]
      kommentti = request.form["kommentti"]
      if len(nimi.split())==0 or len(tekija.split())==0 or len(url.split())==0 or len(kommentti.split())==0:
         return render_template("error.html", viesti="Kaikki kentät tulee täyttää")
